...
export default function id() {
  ...
  async function UpdatePlan(values: any, helpers: any) {
    try {
      setDisabled(true);
      setErrors(undefined);
      //console.table(values);
      const { data } = await axiosInstance.post('plans/update', values);
      alert.show({
        text: data?.message,
        visible: true
      });
  ...
  } = useForm({ initialValues, submitHandler: UpdatePlan });
